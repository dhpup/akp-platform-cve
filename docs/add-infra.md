...
[akp-infra-cve](https://github.com/example-org/akp-infra-cve) repo and the
[`akuity/akp` Terraform provider](https://registry.terraform.io/providers/akuity/akp/latest).

akp-infra-cve is its own self-guided journey, applied in order:
...
- **Fresh start:** run the akp-infra-cve journey first (steps 1–3), then come back
...
- **Already created everything in the Akuity UI:** you can keep it that way —
  ...
  under Terraform, akp-infra-cve's `docs/importing-existing.md` walks through
...
rotating settings) are documented in akp-infra-cve's `docs/day-2.md`.
